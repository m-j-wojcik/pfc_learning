...
print ('Generating Supplementary Figure 6... ')
print ('')

for i in range(4):
    fig, gs = creat_plot_grid(1, 1, 4)
    ax = fig.add_subplot(gs[0], projection='3d')
    plot_state_space_3d(ax=ax, sel_matrix=sel[i], conds=conds, title=titles[i], experiment=2)
    plt.savefig(config['PATHS']['out_template_figures'] + 'supp_fig_6_' + str(i+1) + '.svg')
    plt.savefig(config['PATHS']['out_template_figures'] + 'supp_fig_6_' + str(i+1) + 'png', dpi=300)
...
yaxis_label_lis_r2 = ['set*context (xor 2)', 'context', 'set*context (xor 2)', 'context']
xaxis_label_lis_r3 = ['set*context (xor 2)', None, 'set*context (xor 2)', None]
...
for i_panel in range(4):
    plot_scatter(gs=gs[0, i_panel], fig=fig, x=x_data_r2[i_panel], y=y_data_r2[i_panel], scale=6,
                 ...
                 kde_smooth=1.2, out_factor=1.5)
    # if i_panel is odd
    if i_panel % 2 == 1:
        plot_cov(gs=gs[1, i_panel], fig=fig, dat=cov_dat[i_panel], scale=3, labels=['set', 's*c', 'cxt'], title='covariance')
...
plt.tight_layout()
plt.savefig(config['PATHS']['out_template_figures'] + 'supp_fig_6_5.svg')
plt.savefig( config['PATHS']['out_template_figures'] + 'supp_fig_6_5.png', dpi=300)
print('')
